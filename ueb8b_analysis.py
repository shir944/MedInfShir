...
#import ueb3_vshow as vf
import ueb7c_imgprocess as vf
import ueb7b_diagramm as dia
...
class VideoWindow(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.playback_timer = QtCore.QTimer(parent=self)
    ...
    def open_video(self, file):
        ...
        self.databuf = np.full((self.file.length, 2), np.nan)
    ...
    def show_frame(self, n):
        try:
            self.frame_no = n
            self.file.process_frame(self.frame_no)
            com = self.imgproc.process(self.file.img_buffer)
            self.results.store_data(n, com)
            
            self.plot_widget.set_pos(self.results.get_pos())
            self.databuf[n,:] = self.imgproc.process(self.file.img_buffer)
            self.plot_widget.set_pos(self.databuf)
...
from ueb7c_imgprocess import VideoFile, IMGProcess, Results
from ueb8_analysis import Diagramm, AnalysisWidget
...
class AnalysisWindow(QtWidgets.QWidget):
    def __init__(self, **kwargs):
        super(AnalysisWindow, self).__init__(**kwargs)
        self.init_ui()
        
    def init_ui(self):
        return

import ueb6b_patient_relational as db        
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, **kwargs):
        ...
        self.analysis = AnalysisWindow(parent=self)
